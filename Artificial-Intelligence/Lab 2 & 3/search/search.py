...
def generalGraphSearch(problem, frontier):
    ...
    exploredList = []   #Used to check if node already visited
    frontier.push([(problem.getStartState(), "Stop", 0)])

    while not frontier.isEmpty():   #While all nodes havent been visited
        ...
        if problem.isGoalState(s):  #Check if current node is Goal
            return [x[1] for x in path][1:]

        if s not in exploredList:
            exploredList.append(s)
            
            for successor in problem.getSuccessors(s):
                if successor[0] not in exploredList:
